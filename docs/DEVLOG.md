...
## 6.13 画面レイアウト刷新(旧クライアント踏襲をやめ近代化)

- **課題**: 入力欄が最下部・マップが最上部で上下移動が面倒、ログが手動スクロール必要。
- **刷新**(ユーザー選択案): ページ全体は100vh固定・スクロールさせず各パネル独立スクロール。
  - **デスクトップ**: サイドバイサイド grid。左=マップ(上)+ステータス/リスト(下)、右=チャット(ログ上+入力下)。入力はログの真下・マップ見ながら入力可・上下移動不要。
  - **モバイル(<=820px)**: チャット(ログ+入力)上 → マップ → ステータス下 の縦積み。入力はチャット内最下部で常時表示。
